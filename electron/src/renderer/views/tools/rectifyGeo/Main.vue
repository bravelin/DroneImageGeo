<!--经纬度纠偏-->
<template>
    <div class="page rectify-geo-page">
        <div class="panel-tools">
            <div class="left">
                ...
                <div class="tool-button file-button" :class="{ disabled: currStatus==1 }">2.选择处理完图片存放目录<input type="file" ref="distImagePathObj" directory webkitdirectory @change="doSelectDistImagePath()"/></div>
            </div>
            <div class="right">
        ...
        <div class="list-wrap" v-show="images.length">
            <div class="data-table">
                ...
                <ul v-for="(img,index) in images" :key="index">
                    ...
                    <li>{{ img.lng }}</li>
                    <li>{{ img.lat }}</li>
                    <li class="status-1">{{ img.newLng }}</li>
                    <li class="status-1">{{ img.newLat }}</li>
    ...
    export default {
        ...
        methods: {
            ...
            doRun (index) {
                ...
                if (index < that.images.length) {
                    ...
                    that.$ajax({
                        ...
                        params: { lat: imgObj.lat, lng: imgObj.lng }
                    }).then(res => {
                        if (res.code == 200 && res.data.lat) {
                            const { lat, lng } = res.data
                            // console.log('success...', lat, lng)
                            that.writeGeoDataIntoImg(imgObj, { lat, lng, height: imgObj.h }, index)
                        } else {
            ...
            writeGeoDataIntoImg (imgObj, geo, index) {
                ...
                imgObj.status = 2
                imgObj.newLat = geo.lat
                imgObj.newLng = geo.lng
                imgObj.h = geo.height
                that.currAddAmount++
...
<style lang="scss">
    @import '../../../style/views/toolsRectifyGeo.scss';
</style>
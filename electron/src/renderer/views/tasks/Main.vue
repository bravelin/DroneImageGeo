            ...
            <template v-if="dataList.length">
                <ul v-for="item in dataList" :key="item.id">
                    ...
                    <li>
                        ...
                        <div class="dialog-table-button" v-if="item.status!='1' && item.status!='0'" @click="doPreviewTilesImage(item)">预览瓦片图</div>
                    </li>
                </ul>
            </template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 预览瓦片图
            doPreviewTilesImage ({ id }) {
                this.$store.dispatch(ns.TASKS + '/' + types.TASKS_SWITCH_VIEW_TILES_IMG_VISIBLE_SYNC, {
                    id, isShow: true
                })
            }
        }
